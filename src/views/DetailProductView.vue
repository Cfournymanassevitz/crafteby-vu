...
onMounted(async () => {
  const fetch = useFetch(`${API_URL}/products/${id}`)
  await fetch.json()
...
<template>
  <div class="productid">
    <div class="hero hero-content flex-col lg:flex-row bg-base-200 border rounded-3xl border-vermillon-300 p-3 "
         v-if="product">
      ...
      <div>
        ...
        <button class="btn btn-primary"
                @click="addToCart(product)">Ajouter au panier
        </button>
      </div>
    </div>
  </div>
